...
struct dhcp_option dhcp_options[] = {
	...
	{"logsvr",	OPTION_IP | OPTION_LIST | OPTION_REQ,	0x07},
	...
	{"nisdomain",	OPTION_STRING,				0x28},
	{"nissrv",	OPTION_IP | OPTION_LIST,		0x29},
...
uint8_t *get_option(struct dhcpMessage *packet, int code, unsigned *option_len)
{
	int i, length;
	uint8_t *optionptr;
	...
	while (!done) {
		if (i >= length) {
			DEBUG(LOG_WARNING, "bogus packet, option fields too long.");
			return NULL;
		}
...
int add_option_string(uint8_t *optionptr, uint8_t *string)
{
	...
	if (end + string[OPT_LEN] + 2 + 1 >= 308) {
		DEBUG(LOG_ERR, "Option 0x%02x did not fit into the packet!", string[OPT_CODE]);
		return 0;
	}
