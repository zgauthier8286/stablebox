...
static char **fill_envp(struct dhcpMessage *packet)
{
	...
	if (packet == NULL)
		...
		for (i = 0; dhcp_options[i].code; i++)
			if (get_option(packet, dhcp_options[i].code, 0)) {
				num_options++;
				if (dhcp_options[i].code == DHCP_SUBNET)
					num_options++; /* for mton */
			}
		if (packet->siaddr) num_options++;
		if ((temp = get_option(packet, DHCP_OPTION_OVER, 0)))
			over = *temp;
	...
	for (i = 0; dhcp_options[i].code; i++) {
		...
		if (!(temp = get_option(packet, dhcp_options[i].code, &option_len)))
			continue;
		if (dhcp_options[i].code == DHCP_VENDOROPTS)
		{
			if (client_config.vendor_options)
			{
				FILE *vendoropts = bb_xfopen(client_config.vendor_options, "w+");
				fwrite(temp, option_len, 1, vendoropts);
				fclose(vendoropts);

				envp[j] = xmalloc(strlen(client_config.vendor_options) + 12);
				envp[j++] = bb_xasprintf("vendoropts=%s", client_config.vendor_options);
			}
			continue;
		}
