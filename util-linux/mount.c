	...
	if (optind+2 == argc) {
		...
		mtpair->mnt_opts = cmdopts;
 		if (ENABLE_FEATURE_MOUNT_LABEL) {
			resolved_spec = resolve_mount_spec(&mtpair->mnt_fsname);
 		}
				...
				sizeof(bb_common_bufsiz1)/2))
		{
			...
			if (optind != argc) {
				...
				append_mount_options(&(mtcur->mnt_opts),cmdopts);
 				if (ENABLE_FEATURE_MOUNT_LABEL) {
					resolved_spec = resolve_mount_spec(&mtcur->mnt_fsname);
 				}
...
clean_up:
	...
	if (ENABLE_FEATURE_CLEAN_UP) {
		...
		if (ENABLE_FEATURE_MOUNT_LABEL && resolved_spec) {
         		free(mtcur->mnt_fsname);
 		}
	}
