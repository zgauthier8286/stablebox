...
static void shutdown_system(void)
{
	...
	message(MAYBE_CONSOLE | LOG, "The system is going down NOW !!");
	sync();

	/* Send signals to every process _except_ pid 1 */
	message(MAYBE_CONSOLE | LOG, init_sending_format, "TERM");
	kill(-1, SIGTERM);
	sleep(1);
	sync();

	message(MAYBE_CONSOLE | LOG, init_sending_format, "KILL");
	kill(-1, SIGKILL);
	sleep(1);
...
static void shutdown_signal(int sig)
{
	...
	message(MAYBE_CONSOLE | LOG, "Requesting system %s.", m);
	sync();
